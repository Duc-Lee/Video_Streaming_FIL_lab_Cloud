...
class StreamingMeasuring:
    def __init__(self, config):
        ...
        # Optional gateway override; fall back to existing ksvc URL
        self.gateway_url = os.getenv("GATEWAY_URL", None)
        ...
        for replica in self.replicas:
            for rep in range(1, self.repetition + 1, 1):
                ...
                for _ in range(self.curl_time):
                    ...
                    trigger_url = (
                        f"{self.gateway_url}/stream/start/{self.resolution}/10"
                        if self.gateway_url
                        else f"http://{self.ksvc_name}.{self.namespace}.192.168.17.1.sslip.io/stream/start/{self.resolution}/10"
                    )
                    result = get_curl_metrics(url=trigger_url)

                    logging.debug(f"Curl: {trigger_url}")
    ...
    def measure(self):
        ...
        for replica in self.replicas:
            for rep in range(1, self.repetition + 1, 1):
                ...
                logging.info("Start trigger service")
                trigger_url = (
                    f"{self.gateway_url}/stream/start/{self.resolution}/{self.detection_time}"
                    if self.gateway_url
                    else f"http://{self.ksvc_name}.{self.namespace}.192.168.17.1.sslip.io/stream/start/{self.resolution}/{self.detection_time}"
                )
                result = get_curl_metrics(url=trigger_url)
...
class MainThread:
    ...
    def get_bitrate_fps(detection_time, ksvc_name, namespace, result_file):
        ...
        while time.time() - start_time < detection_time:
            ...
            gateway_url = os.getenv("GATEWAY_URL", None)
            status_url = (
                f"{gateway_url}/stream/status"
                if gateway_url
                else f"http://{ksvc_name}.{namespace}.192.168.17.1.sslip.io/stream/status"
            )
            result = query_url(url=status_url)
